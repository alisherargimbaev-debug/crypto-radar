...
// ── НАСТРОЙКИ ───────────────────────────────────────────────────────────
const MIN_PNL_MONTH_PCT = 50;                   // PnL за месяц, % (снижено с 100 — больше трейдеров)
const MIN_ACCOUNT_USD   = 100_000;              // минимальный размер аккаунта (снижено с 250k)
const MIN_POSITION_USD  = 100_000;             // фильтр размера позиции (снижено с 1M — видим реальные позиции)
const MAX_TRACKED       = 30;  // расширили с 15 до 30                   // сколько трейдеров держим одновременно
const CHECK_INTERVAL_MS      = 10 * 60 * 1000;  // каждые 10 минут (было 5 — слишком часто для Hyperliquid)
...
const MAX_REQS_PER_MIN       = 15;              // rate-limit (снижено с 30 — Hyperliquid банит)
...
async function httpPost(url, body, extraHeaders = {}) {
  await throttle();
  await new Promise(r => setTimeout(r, 2000)); // минимум 2с между запросами
  try {
